...
namespace MugoWeb\PageBundle\Services;
...
class MugoPageService
{
    ...
    function __construct(
        ...
        TwigThemeLoader $twigThemeLoader,
        ...
        $this->twigThemeLoader = $twigThemeLoader;
        $this->locationService = $locationService;
        $this->repository = $this->container->get( 'ibexa.api.repository' );
    }

    public function createMugoPageTable(){
        ...
        'mysql://mugoca:hVZQ7e3D@127.0.0.1:3306/mugo2020_ca?serverVersion=8&charset=utf8mb4';
    ...
    private function prepareLayoutData(Array $layoutData){
        ...
        if (isset($layoutsConfig[$layoutData['layout']]) && $layoutsConfig[$layoutData['layout']] && isset($layoutsConfig[$layoutData['layout']]['data']) && $layoutsConfig[$layoutData['layout']]['data']){
            ...
            // test template
            if ($this->twigThemeLoader->exists( '@' . $config->template)){
                $config->has_valid_template = true;
            } else {
                $config->has_valid_template = false;
            }
    ...
    private function prepareZonesData(Array $zonesData){
        ...
        foreach ($zonesData as $zoneData) {
            ...
            if (isset($zonesConfig[$zoneData['identifier']]) && $zonesConfig[$zoneData['identifier']] && isset($zonesConfig[$zoneData['identifier']]['data']) && $zonesConfig[$zoneData['identifier']]['data']) {
                ...
                $config->template = ($zonesConfig[$zoneData['identifier']]['data']['template'] ?? '');
        ...
        foreach ($blocksData as $blockData) {
            ...
            if (isset($blockData['type']) && $blockData['type']
                ...
                if(isset($blockData['content']['related_content']) && $blockData['content']['related_content']) {
                    foreach ($blockData['content']['related_content'] as $relatedContentItem){
                        $locationId = $relatedContentItem['locationId'];
                        $relatedContentId = $relatedContentItem['contentId'];
                        try {
                            $content = $this->contentService->loadContent($relatedContentId);
                            if(!$content->versionInfo->contentInfo->isTrashed()) {
                                $relatedContentArray[] = array(
                ...
                if($config->attr){
                    ...
                    foreach ($config->attr as $configAttribute){
                                ...
                                if ($dataAttribute['identifier'] == $configAttribute['identifier']) {
                                    switch ($configAttribute['type']) {
                                        ...
                                        case 'contentrelation':
                                            ...
                                            foreach ($dataAttribute['value'] as $relatedContentItem){
                                                $locationId = $relatedContentItem['locationId'];
                                                $relatedContentId = $relatedContentItem['contentId'];
                                                try {
                                                    $content = $this->repository->sudo(
                                                        function ( $repository ) use ( $relatedContentId ) {
                                                            return $this->contentService->loadContent($relatedContentId);
                                                        }
                                                    );

                                                    if(!$content->versionInfo->contentInfo->isTrashed()) {
                                                        $customAttribute['value'][] = array(
                                                                'name' => $content->getName(),
                                                                'content_id' => $content->id,
                                                                'location_id' => $locationId,
                                                                'identifier' => $content->getContentType()->identifier,
                        ...
                        $customAttributes[$customAttribute['identifier']] = $customAttribute;
